...
- (BOOL) insert
{
	...
	NSString *findSql = [NSString stringWithFormat:@"insert into bbcdetail(Voaid,ParaId,IdIndex,Timing,Sentence,ImgWords,ImgPath,Sentence_cn) values(%d,%d,%d,%d,\"%@\",\"%@\",\"%@\",\"%@\") ;",self._voaid,self._paraid,self._idIndex,self._timing,self._sentence,self._imgWords,self._imgPath,self._sentence_cn];
...
+ (id) find:(NSInteger) voaid{
	...
	id<PLResultSet> rs;
	NSString *findSql = [NSString stringWithFormat:@"select * FROM bbcdetail WHERE voaid = %d", voaid];
...
//    NSLog(@"删除内容-%d",voaid);
    PLSqliteDatabase *dataBase = [database setup];
    NSString *findSql = [NSString stringWithFormat:@"delete from bbcdetail where voaid = %d ;",voaid];
    if([dataBase executeUpdate:findSql]) {
    }
}
